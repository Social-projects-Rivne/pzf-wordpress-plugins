...
$categories = get_the_terms($post->ID, 'vh_categories');
$category_string = '';
foreach ($categories as $value) {
	$category_string .= $value->name.', ';
}
$category_string = substr($category_string, 0, -2);

$entry_utility .= '<span class="icon-tag-1">'.$category_string.'</span>';
...
<div class="entry no_left_margin first-entry <?php echo $is_author_desc; ?> <?php if ( !isset($img[0]) ) { echo ' no-image'; } ?><?php echo (LAYOUT != 'sidebar-no') ? ' vc_col-sm-12' : ' vc_col-sm-12'; ?>">
	<div class="entry-image vh_animate_when_almost_visible with_full_image <?php echo $vh_blog_image_layout . $is_author_desc; ?>"<?php echo $clear; ?>>
		...
		<div class="entry-content">
				<?php
					echo '
					<div class="page_title">' . get_the_title() . '</div>
					<div class="open-blog-social">
						<div class="social-facebook">
							...
							<div class="fb-like" data-href="' . get_permalink( $post_id ) . '" data-layout="button_count" data-action="like" data-show-faces="true" data-share="false"></div>
						</div>
			...
			if ( is_search() ) {
				...
				echo '<div class="animals-open-content">';
