...
<div class="page-<?php echo LAYOUT; ?> page-wrapper">
	...
	if ( get_post_type( $post ) == 'post' ) { ?>
		...
		echo vh_breadcrumbs() . 
		'<h1 class="blog_title">' . __( 'Blog', 'vh' ) . '</h1></div>';
				...
				<div class="main-inner">
					<div class="vc_row wpb_row vc_row-fluid">
						...
						if ( have_posts() ) {
							while ( have_posts() ) {
								the_post();
								get_template_part( 'content-animals_places', 'single' ); 
								if ( get_post_type( $post ) == 'post' ) { ?>
									<div class="clearfix"></div>
												...
												if (!empty( $prev_post )) { ?>
													<div class="nav_button left">
														<div class="prev-post-img">
															<?php echo '<a href="' . get_permalink( $prev_post->ID ) . '">'.get_the_post_thumbnail( $prev_post->ID, 'open-small' ).'</a>'; ?>
														</div>
												...
												if (!empty( $next_post )) { ?>
													<div class="nav_button right">
														<div class="next-post-img">
															<?php echo '<a href="' . get_permalink( $next_post->ID ) . '">'.get_the_post_thumbnail( $next_post->ID, 'open-small' ).'</a>'; ?>
														</div>
