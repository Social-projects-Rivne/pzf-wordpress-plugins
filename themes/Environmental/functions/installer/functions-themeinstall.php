...
<div class="updated vhman-message quick-setup">
	...
	<!-- Step 1 -->
		...
		Installs Page Builder, Environmental functionality, etc.
		</span></span><br />
	...
	<!-- Step 3 -->
		<?php
			$step_link = '';
			if ( !get_option( SHORTNAME . '_required_plugins_installed') ) {
				echo '<p id="theme-setup-step-3" class="submit step-revolution step-inactive">';
				$step_link = 'javascript:void(0)';
			} else {
				echo '<p id="theme-setup-step-3" class="submit step-revolution step-active">';
				$step_link = admin_url() . "admin.php?page=revslider";
			}
		?>
		<span class="step"><span class="number">3</span></span> 
		<span class="step-body">
			<a href="<?php echo $step_link ?>" class="button button-primary">Import Revolution slider</a> 
			<span class="step-description">Import files for this step are located in<br /> 
			Revolution Slider Import file folder </span>
		</span>
		</p>
...
function pvt_wordpress_content_importer() {
	...
	if ( is_admin() && isset($_GET['importcontent']) ) {
		...
		if ( $_GET['importcontent'] == 'alldemocontent' ) {
			...
			$term = get_term_by('name', 'Menu 1', 'nav_menu');
