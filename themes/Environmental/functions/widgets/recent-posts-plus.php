...
class RecentPostsPlus extends WP_Widget {
    ...
    private $default_config = array(
        ...
        <div class="recent-post-plus-date">
        <span class="icon-calendar">{DATE}</span></div>
        <a href="{PERMALINK}" class="news-img no_thickbox latest_posts">{THUMBNAIL}</a>
        </div>
        <div class="post_container">
        <h3><a href="{PERMALINK}">{TITLE}</a><span class="icon-comment-1">{COMMENT_COUNT}</span></h3>
        {EXCERPT}
    ...
    function __construct() {
        ...
        parent::__construct('recent-posts-plus', __('Environmental - Recent Posts Plus', 'vh'), $widget_ops);
    }
    ...
    function parse_output($instance) {
        ...
        if( $the_query->have_posts() ) {
            ...
            while ( $the_query->have_posts() ) { $the_query->the_post();
                ...
                if($include_post_thumbnail == "false" || get_the_post_thumbnail($ID, 'gallery-small') == '' ) {
                    $POST_THUMBNAIL = '';
                    $extra_class = "no_thumb";
                } else {
                    $extra_class = "";
                    if ( $post_thumbnail_width ==  '' || $post_thumbnail_height == '' ) {
                        $POST_THUMBNAIL = get_the_post_thumbnail($ID, 'gallery-small');
                    } else {
                ...
                if ( $post_date_format == '' ) {
                    $post_date_format = get_option('date_format', 'd M');
                }
                ...
                $widget_ouput_template_params = array(
                    ...
                    '{DATE}'          => date_i18n($post_date_format, $post_timestamp),
    ...
    function form( $instance ) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id('count'); ?>"><?php _e('Number of posts to show:', 'vh'); ?></label>
        ...
        <p>
            <?php if( current_theme_supports('post-thumbnails') ): ?>
                ...
                <label for="<?php echo $this->get_field_id('include_post_thumbnail'); ?>"><?php _e('Include post thumbnail', 'vh'); ?></label><br>
            <?php endif; ?>
            ...
            <label for="<?php echo $this->get_field_id('include_post_excerpt'); ?>"><?php _e('Include post excerpt', 'vh'); ?></label><br>
            ...
            <label for="<?php echo $this->get_field_id('show_expert_options'); ?>"><?php _e('Show expert options', 'vh'); ?></label>
        </p>

        <div class="rpp_expert-panel" style="display:none; margin-top: 10px">

            <p>
                <label for="<?php echo $this->get_field_id('truncate_post_title'); ?>"><?php _e('Limit post title:', 'vh'); ?></label>
                ...
                <label for="<?php echo $this->get_field_id('truncate_post_excerpt'); ?>"><?php _e('Limit post excerpt:', 'vh'); ?></label>
                ...
                <label for="<?php echo $this->get_field_id('truncate_elipsis'); ?>"><?php _e('Limit ellipsis:', 'vh'); ?></label>
                ...
                <label for="<?php echo $this->get_field_id('post_date_format'); ?>"><?php _e('Post date format:', 'vh'); ?></label> <a title="View Documentation" target="_blank" href="http://www.pjgalbraith.com/2011/08/recent-posts-plus/">(?)</a>
            ...
            <?php if( current_theme_supports('post-thumbnails') ): ?>
                <p>
                    <label for="<?php echo $this->get_field_id('post_thumbnail_width'); ?>"><?php _e('Thumbnail size (WxH):', 'vh'); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id('wp_query_options'); ?>"><?php _e('WP_Query Options', 'vh'); ?></label> <a title="View Documentation" target="_blank" href="http://www.pjgalbraith.com/2011/08/recent-posts-plus/">(?)</a><br />
            ...
            <p>
                <label for="<?php echo $this->get_field_id('widget_output_template'); ?>"><?php _e('Widget Output Template', 'vh'); ?></label> <a title="View Documentation" target="_blank" href="http://www.pjgalbraith.com/2011/08/recent-posts-plus/">(?)</a><br />
